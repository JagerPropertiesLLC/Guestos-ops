...
export async function GET(_request, { params }) {
  ...
  const [
    ...
    supa.from('swppp_site_check_findings').select('*, site_check:swppp_site_checks!site_check_id(*)').eq('inspection_id', params.id).order('site_check_id'),
    supa.from('swppp_corrective_actions').select('*').eq('inspection_id', params.id)
  ]);

  const buf = await buildInspectionPdf({ inspection, bmpFindings, siteCheckFindings, correctiveActions });
